...
def is_suggested_fix_safe(suggested_fix: str) -> tuple[bool, str]:
    ...
    subprocess_patterns = [
        (r'\b(os\.system|os\.popen|os\.spawn|pty\.spawn)\b', "unsafe os command execution"),
        (r'shell\s*=\s*True', "shell=True in sub-process execution")
    ]
...
"""
    ...
    models_to_try = ['gemini-3.5-flash', 'gemini-3.0-flash', 'gemini-2.0-flash']
    response = None
    success = False
    ...
    try:
        ...
        try:
            ...
            # Safely append missing closing brace if the LLM JSON generation was truncated
